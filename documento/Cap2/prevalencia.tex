...
Teniendo ya una intuición topológica sobre el tamaño de $\mathcal{ND}[0,1]$, en esta sección buscaremos acercarnos a una definición más precisa de este concepto; en primera instancia abordaremos la siguiente pregunta: ¿podemos definir una medida ``útil'' en $\mathcal{C}[0,1]?$
...
\begin{definition}
    ...
    \end{enumerate}
\end{definition}
\begin{definition}\label[definition]{def:timido}
...
\begin{theorem}\label[theorem]{thm:medida_producto}
...
\begin{theorem}[Tonelli]\label[theorem]{thm:Tonelli}
...
\begin{definition}
    ...
    \[\mu(S) = \mu(S\cap P).\]
\end{definition}
\begin{lemma}\label[lemma]{lem:conv_transversal}
...
\begin{theorem}\label[theorem]{thm:Timidos_en_Rn}
...
\end{definition}
\begin{theorem}\label[theorem]{thm:medidas_producto_inf}
