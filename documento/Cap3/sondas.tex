...
\begin{proposition}\label[proposition]{prop:simpl_sonda}
...
\begin{theorem}\label[theorem]{thm:simp_prev}
    Si $T\subseteq V$ tiene una sonda entonces es prevalente. 
\end{theorem}

Otra ventaja de usar sondas para demostrar que un conjunto es prevalente radica en que una misma sonda puede demostrar que un conjunto es prevalente en muchos subespacios diferentes, veamos:
\begin{proposition}
    Si $\mu$ es transversal a $S$ y el soporte de $\mu$ está contenido en un subespacio $P$ de $V$, entonces $S\cap P$ es tímido en $P$.
\end{proposition}

Ahora bien, una forma más intuitiva de entender cómo se comportan las sondas de conjuntos borelianos es la siguiente:
\begin{observation*}
...
Teniendo esto en cuenta, veamos algunos ejemplos que muestran cómo todo esto simplifica la demostración de que un conjunto es prevalente.
\begin{example} 
    Si definimos 
