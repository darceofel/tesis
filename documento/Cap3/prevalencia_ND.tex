...
Complicando aún más nuestro objetivo, en 1936 Stephan Mazurkiewicz probó que $\mathcal{ND}[0,1]$ no es un subconjunto de Borel de $\mathcal{C}[0,1]$ (ver \cite{Mauldin_Non_Borel}). Por esta razón, trabajaremos con otro conjunto un poco más fácil de manejar:
\begin{definition}
...
\end{proposition}
\begin{proof}\mbox{}\\*
    (1) Llamemos \[F = \mathcal{C}[a,b]\setminus \mathcal{NL}_M[a,b]\] y tomemos $f \in \cerradura (F)$. Entonces existe $\{f_n\}_{n\in\mathbb{N}}\subseteq F$ tal que $f_n\xrightarrow{u} f$. Como $f_n\notin \mathcal{NL}_M[a,b]$, entonces para cada $n\in\mathbb{N}$ existe $x_n\in [a,b]$ tal que $f_n$ es $M$-Lipschitz en $x_n$. Es decir, para toda $y\in[a,b]$, 
...
\begin{lemma}\label[lemma]{lem:tecnico_prev_ND}
...
\end{theorem}
\begin{proof}
    Tomemos $g,h\in \mathcal{C}[0,1]$ como en el lema anterior, y $f\in \mathcal{C}[0,1]$. Observemos que
