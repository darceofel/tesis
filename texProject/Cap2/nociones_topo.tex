...
\begin{theorem}\label{theorem:ND_denso}El espacio $\mathcal{ND}[0,1]$ es denso en $(\mathcal{C}[0,1], \; || \cdot ||_{\infty})$.
\end{theorem}
...
\end{definition}
\begin{proposition}
    Sean $(X, \tau)$  un espacio topológico y $A\subset X$. El conjunto $A$ es denso en ninguna parte si y sólo si $X\setminus \cerradura(A)$ es denso en $X$.
\end{proposition} 
...
\begin{definition}
    Sean $(X, \tau)$  un espacio topológico y $A\subset X$. Decimos que $A$ es \textit{de primera categoría} si es unión numerable de conjuntos densos en ninguna parte. Por el contrario, decimos que $A$ es \textit{de segunda categoría} si no es de primera categoría.
\end{definition}
\begin{example}
...
\begin{lemma}\label{lema:poligonos_denso}
